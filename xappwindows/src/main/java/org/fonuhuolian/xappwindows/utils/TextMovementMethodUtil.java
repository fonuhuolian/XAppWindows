...
public class TextMovementMethodUtil extends LinkMovementMethod {

    private ClickSpanUtil mTextClickSpan;
    ...
    public boolean onTouchEvent(TextView widget, Spannable buffer, MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            ClickSpanUtil touchTextClickSpan = getTextSpan(widget, buffer, event);
     ...
     * @return
     */
    private ClickSpanUtil getTextSpan(TextView widget, Spannable spannable, MotionEvent event) {
        ...
        ClickSpanUtil[] link = spannable.getSpans(off, off, ClickSpanUtil.class);
        ClickSpanUtil touchedSpan = null;
        if (link.length > 0) {
            touchedSpan = link[0];
        }
        return touchedSpan;
